...
import { cx, DISPOSITIONS, DATA_SCOPE, FHIR_MAP, EHR_FLOW, EHR_FLOW_SIMPLE, REFERENCES, BOUNDARY, PLATFORM_POTENTIAL, WITHOUT_WITH } from "../data";
...
export default function HowItWorks() {
  ...
  return (
    <div>
      ...
      {/* three disposition families */}
      <Card className="mb-6">
        <CardHead accent="bg-amber-500" eyebrow="Taxonomy" title="How a reviewed case can end — three different things" />
        ...
        <div className="border-t border-slate-100 bg-slate-50 px-5 py-3 text-xs leading-relaxed text-slate-600">
          Only the first family counts against case-finding quality. Collapsing the other two into "false positives" would
      ...
      <Card className="mb-6">
        ...
        <div className="grid grid-cols-1 divide-y divide-slate-100 lg:grid-cols-2 lg:divide-x lg:divide-y-0">
          <div className="px-5 py-4">
            <Eyebrow>Conceptual flow</Eyebrow>
            <ol className="mt-3 space-y-1">
              {EHR_FLOW_SIMPLE.map((f, i) => (
                <li key={f.step}>
                  <div
                    className={cx(
                  ...
                  {i < EHR_FLOW_SIMPLE.length - 1 && <ArrowDown size={13} className="mx-auto my-0.5 text-slate-300" />}
                </li>
              ))}
            </ol>
            <p className="mt-3 text-xs leading-relaxed text-slate-500">
              Violet is machine work. Teal is a person. Everything after the check is human.
            </p>
          </div>
          <div className="px-5 py-4">
        ...
        {techOpen && (
          <div className="border-t border-slate-100 px-5 py-4">
            <Eyebrow>Detailed workflow</Eyebrow>
            <ol className="mb-4 mt-2 flex flex-wrap gap-x-2 gap-y-1 text-xs text-slate-600">
              {EHR_FLOW.map((step, i) => (
                <li key={step} className="flex items-center gap-2">
                  <span className="rounded bg-slate-100 px-2 py-0.5">{step}</span>
                  {i < EHR_FLOW.length - 1 && <span className="text-slate-300">→</span>}
                </li>
              ))}
            </ol>
      ...
      <Card className="mb-6">
        <CardHead accent="bg-slate-400" eyebrow="The difference" title="What changes after the report is signed" />
        <div className="grid grid-cols-1 divide-y divide-slate-100 lg:grid-cols-2 lg:divide-x lg:divide-y-0">
          {[
            ["Without FractureBridge", WITHOUT_WITH.without, false],
            ["With FractureBridge", WITHOUT_WITH.with, true],
          ].map(([title, steps, good]) => (
            <div key={title} className="px-5 py-4">
              <div className={cx("font-mono text-xs uppercase tracking-wider", good ? "text-teal-700" : "text-slate-400")}>{title}</div>
              <ol className="mt-2 flex flex-wrap items-center gap-x-1.5 gap-y-2">
                {steps.map((st, i) => (
                  <li key={st} className="flex items-center gap-1.5">
                    <span className={cx("rounded-md px-2 py-1 text-xs", good ? "bg-teal-50 text-teal-900" : "bg-slate-100 text-slate-600")}>{st}</span>
        ...
        <div className="border-t border-slate-100 px-5 py-3 text-xs leading-relaxed text-slate-500">
          The difference is accountable workflow: an owner and a documented outcome. No claim is made that a fracture is
          prevented or that a care gap is eliminated.
        </div>
      </Card>
      ...
      <Card className="mt-6 border-dashed">
        ...
        <div className="px-5 py-4">
          <p className="text-sm leading-relaxed text-slate-600">{PLATFORM_POTENTIAL}</p>
        </div>
      </Card>
    </div>
  );
