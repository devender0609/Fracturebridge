...
export default function Measures() {
  ...
  const [defOpen, setDef] = useState(null);
    ...
    <div>
      <header className="mb-5">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600">
          Where cases are stuck, whether the right cases are found, whether a team can carry the work, whether follow-up
          progresses, and what would need a longer study. The default view is the measurement framework; the
          illustrative scenario shows simulated values so the charts can be seen, and contains no results.
        </p>
      </header>
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-slate-100 border-b border-slate-100 lg:grid-cols-4 lg:divide-y-0">
            ...
            <Kpi value={TO_MEASURE} label="Share of routed cases confirmed actionable" sub="after human review" />
            <Kpi value={TO_MEASURE} label="Screening-exclusion rate" sub="not-for-pathway only" />
          </div>
        ...
        <Card>
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-slate-100 border-b border-slate-100 lg:grid-cols-4 lg:divide-y-0">
            ...
            <Kpi value={TO_MEASURE} label="Time to human review" sub="median, from report signature" />
            <Kpi value={TO_MEASURE} label="Time to owner assignment" sub="from review" />
            <Kpi value={TO_MEASURE} label="Reviewer effort per case" sub="median, self-timed" />
          </div>
          <div className="px-5 py-4">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="px-5 py-4">
              ...
              <dl className="mt-3 divide-y divide-slate-100">
                {PROCESS_DEFINITIONS.map(([step, def]) => (
                  <div key={step} className="py-2">
                    <dt className="flex items-center justify-between gap-3">
                      <span className="text-sm font-medium text-slate-800">{step}</span>
                      <button
                        onClick={() => setDef(def === defOpen ? null : def)}
                        aria-expanded={defOpen === def}
                        className="shrink-0 font-mono text-xs uppercase tracking-wider text-teal-800 hover:underline"
                      >
                        {defOpen === def ? "Hide" : "View definition"}
                      </button>
                    </dt>
                    {defOpen === def && <dd className="mt-1 text-xs leading-relaxed text-slate-500">{def}</dd>}
                  </div>
                ))}
              </dl>
              <p className="mt-2 text-xs leading-relaxed text-slate-500">
