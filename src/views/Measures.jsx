  ...
  return <div>
    <header className="mb-6">
      ...
      <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600">
        The prototype shows what a pilot would measure. Values shown as numbers are illustrative only; dashes remain intentionally unfilled until real pilot data exist.
      </p>
    </header>

    <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
      <strong>Illustrative pilot dashboard.</strong> Demonstration values only — not Ascension performance data.
    </div>
    ...
    <div className="grid gap-6 lg:grid-cols-3">
      {sections.map(({icon:Icon,accent,eyebrow,title,metrics})=><Card key={title}>
        ...
        <div className="grid grid-cols-2 divide-x divide-y divide-slate-100">
          {metrics.map(([label,n,sub])=><Stat key={label} n={n} label={label} sub={sub}/>)}
        </div>
      </Card>)}
    </div>
    ...
    <Card className="mt-6">
      ...
      <div className="flex gap-2 border-t border-slate-100 bg-amber-50 px-5 py-3 text-sm text-amber-900">
        <AlertTriangle size={16} className="mt-0.5 shrink-0"/>
        <span>A proposed 6–12 month pilot should focus on case-finding performance, operational burden, and care-process completion. It should not claim that FractureBridge has already reduced future fractures.</span>
      </div>
    </Card>
    ...
    <Card className="mt-6 p-5">
      ...
      <p className="mt-2 text-sm leading-relaxed text-slate-600">
        The pilot should measure weekly case volume and median reviewer time to determine staffing requirements before scale-up. The prototype does not assume that an existing team can absorb a particular volume.
      </p>
    </Card>
  </div>;
}
